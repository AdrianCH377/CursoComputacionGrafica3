...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Practica9 -Fuentes de luz- Cruz Hernandez Santiago Adrian", nullptr, nullptr);
	...
	Model mapa((char*)"Models/Mountain.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(6.0f, 5.0f, 19.0f));
		model = glm::scale(model, glm::vec3(12.5f, 12.5f, 12.5f));
		glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
		red_dog.Draw(lightingShader);
		glBindVertexArray(0);

		model = glm::translate(model, glm::vec3(0.2f, 1.0f, 0.6f));
		...
		model = glm::translate(model, glm::vec3(0.3f, -2.0f, 0.5f));
		...
		model = glm::scale(model, glm::vec3(2.3f)); // Make it a smaller cube
